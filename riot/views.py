...
def user_info(request, server, summoner_name, template='riot/user-profile.html'):
    ...
    user_account_info, summoner_stats = api_interaction.get_ranked_stats(server, summoner_name)
...
def champ_info(request, server, summoner_name, champion_name, template= 'riot/user-champ.html'):
    # If user submits the form, it will redirect to the user profile page
    if ('summoners_name' and 'server') in request.POST:
        summoner_name = request.POST['summoners_name']
        server = request.POST['server']
    ...
    user_account_info, summoner_stats = api_interaction.get_champion_stats(server, summoner_name, champion_name, champ_json)
    ...
    if user_account_info['success']:
        ...
        champ_games = api_interaction.get_champion_matchlist(server, account_id, champion_id)
...
def in_game(request, server, summoner_name):
    ...
    game_info, blue_players, red_players = api_interaction.in_game_info(server, summoner_id, champ_json)
